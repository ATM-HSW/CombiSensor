...
CircuitPython library for the Adafruit SGP4x Air Quality Sensor / VOC Index Sensor Breakouts
...
* SGP41 VOC/NOx Air Quality Sensor
...
_READ_CMD_VOC = b"\x26\x0F\x80\x00\xA2\x66\x66\x93"
_READ_CMD_VOC_NOx = b"\x26\x19\x80\x00\xA2\x66\x66\x93"
...
class SGP4x:
    """
    Class to use the SGP41 Air Quality Sensor

    :param int address: The I2C address of the device. Defaults to :const:`0x59`


    **Quickstart: Importing and using the SGP40/41 gas sensor**

        Here is one way of importing the `SGP4x` class so you can use it with the name ``sgp``.
    ...
    .. note::
        The operational range of temperatures for the SGP4x is -10 to 50 degrees Celsius
        and the operational range of relative humidity for the SGP4x is 0 to 90 %
    ...
    def __init__(self, i2c, address=0x59):
        ...
        self._measure_command = _READ_CMD_VOC_NOx
    ...
    def initialize(self):
        ...
        featureset = self._read_word_from_command(delay_ms=10)
        if featureset[0] == 0x3220:   # documented SGP40 value
            self.SGP4x = _SGP40
            print("SGP40")
        elif featureset[0] == 0x240:  # undocumented SGP41 value
            self.SGP4x = _SGP41
            print("SGP41")
        else:                         # unknown value
            raise RuntimeError("Feature1 set does not match 0x3220(SGP40) or 0x240(SGP41): %s" % hex(featureset[0]))
    ...
    def _celsius_to_ticks(temperature):
        ...
        Temperature to Ticks : From SGP4x Datasheet Table 10
    ...
    def _relative_humidity_to_ticks(humidity):
        ...
        Relative Humidity to Ticks : From SGP4x Datasheet Table 10
    ...
    def raw(self):
        ...
        if self.SGP4x == _SGP40:
            read_value = self._read_word_from_command(delay_ms=50)
        elif self.SGP4x == _SGP41:
            read_value = self._read_word_from_command(delay_ms=50, readlen=2)
        self._command_buffer = bytearray(2)
        return read_value[0]
    ...
    def measure_raw(self, temperature=25, relative_humidity=50):
        ...
        if self.SGP4x == _SGP40:
            ...
            raise RuntimeError("Unkown sensor: %s")
    ...
    def execute_conditioning(self, temperature=25, relative_humidity=50):
        """
        A humidity and temperature compensated raw gas value which helps
        address fluctuations in readings due to changing humidity.
        ...
        """
        # recycle a single buffer
        _compensated_read_cmd = [0x26, 0x12]
        humidity_ticks = self._relative_humidity_to_ticks(relative_humidity)
        humidity_ticks.append(self._generate_crc(humidity_ticks))
        temp_ticks = self._celsius_to_ticks(temperature)
        temp_ticks.append(self._generate_crc(temp_ticks))
